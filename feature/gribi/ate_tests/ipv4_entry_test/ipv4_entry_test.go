...
// TODO / DO NOT SUBMIT: Do we support running the test without FIBACK support?
...
// TestIPv4Entry tests a single IPv4Entry forwarding.
func TestIPv4Entries(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			t.Log("new test", tc.desc)
			c := fluent.NewClient()
			c.Connection().WithStub(gribic).
				// TODO / DO NOT SUBMIT: Should we use WithPersistence() ?
